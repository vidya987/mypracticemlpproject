...
IdentityFile ~/.ssh/id_rsa
  ...
  ** Clone repository at Gitbash **
    ...
    * `pwd` -- Check and verify that the current working directory is C:\users\Hvuser/workspace
    * `git clone ssh://git-codecommit.us-east-2.amazonaws.com/v1/repos/ftp72 `
    * `cd ftp72`
    * `git status`
  * open Visual Studio (VS) Code
    * Open folder c:\users\Hvuser\workspace\ftp72
...
# Workshop material - Day #2 - MySQL Workbench/Cli/Curl/Browser to interact w/ application
  ...
  * `CREATE DATABASE ftp72;` and click the lightning button
  * `CREATE USER 'ftp72'@'localhost' IDENTIFIED BY 'ftp72';`
  * `GRANT ALL ON ftp72.* TO 'ftp72'@'localhost';`
  ...
  * go to gitbash, ensure you are in workspace/ftp72
  ...
  * Build the war file as follows:
    ...
    * `cp target/ftp72-0.0.1-SNAPSHOT.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/ftp72.war`
    * start tomcat and tail its logs
      * `cd D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16`
      * `rm -rf logs/*`
      * `./bin/startup.sh`
      * `tail -f logs/*`
      * `curl -vvv http://localhost:8080/ftp72/api/employees | python -m json.tool`
      * `curl -vvv http://localhost:8080/ftp72/api/employees/2000 | python -m json.tool`
   * `cd ../../webui/lm-app/`
   * `npm install`
   * `ng build`
   * `cp ../../restservice/leavemanager/target/ftp72-0.0.1-SNAPSHOT.war ./ftp72.war`
   * `cd dist`
   * `jar -uvf ../ftp72.war *`
   * `cd ..`
   * `jar -tvf ftp72.war`
   * `cp ftp72.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/ftp72.war`
   * Notice that the tail terminal shows that the new version of the web application archive is now getting deployed
   * Open Chrome browser and navigate to http://localhost:8080/ftp72/
...
# Workshop material - Day #4 - Configure Project Board in Trello for  Managing and Tracking Your Project
  ...
  * Create a board for your project as "ftp72"
  ...
  * [Team member #3]
     * Create "Current Sprint" with "ftp72 Sprint 1" as label 
